...
MENU_LIST = {
    ...
    "単品": ["王道のオムライス", "特製パラパラ炒飯", "カルボナーラパスタ", "カツ丼", "牛丼", "ソース焼きそば", "カレーうどん", "ミートソーススパゲティ", "親子丼"],
    "手軽": ["TKG（卵かけご飯）", "納豆キムチご飯", "サバ缶キャベツ和え", "冷凍うどんの明太バター", "レトルトカレー＆チーズ", "サラダチキン丼", "カップヌードル（背徳飯）"],
    "外食": ["広島風お好み焼き", "魚介系醤油ラーメン", "回転寿司", "サイゼリヤ", "吉野家", "マクドナルド", "近所の居酒屋", "焼肉屋", "ココイチ"]
}
...
def handle_message(event):
    ...
    if text in MENU_LIST:
        ...
        if text == "献立":
            reply = f"今日のバッチリ【{text}】はこれや！\n\n{menu}"
        else:
            reply = f"今日の【{text}】は【 {menu} 】で決まりや！"
